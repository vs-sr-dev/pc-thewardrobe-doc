...
def main():
    ...
    for p in args.paths:
        if os.path.isdir(p) and args.recurse:
            ...
            # The same defect ne.py carried: without --recurse a directory
            # fell through to open() and raised an uncaught PermissionError.
            # The guard is shared and it names the flag that would have done
            # what the caller meant.
            sys.exit("jpeg: %s is a directory; this reader wants one file. "
                     "Add --recurse to walk it for .jpg, .jpeg and .jpe."
                     % p)
        else:
            dirguard.want_file(p, "jpeg")
            files.append(p)
